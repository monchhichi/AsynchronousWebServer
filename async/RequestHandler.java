...
public class RequestHandler implements IReadWriteHandler {
  ...
  private void updateState(SelectionKey key) throws IOException {
    ...
    /*
     * if (responseSent) { Debug.DEBUG("***Response sent; shutdown connection"); client.close();
  ...
  private void processInBuffer(SelectionKey key) throws IOException {
    ...
    Debug.DEBUG("handleRead: Read data from connection " + client + " for " + readBytes
  ...
  private void generateResponse() {
    try {
      ...
      if (httpRequest.getUrl().equals("/load")) {
        ...
        // map URL to File
        // iPhone
        if (httpRequest.getHeaderUserAgent() != null
            && httpRequest.getHeaderUserAgent().contains("iPhone"))
